...
const handleDeleteEx = (id) => {
  ...
  return (
    ...
    {nameVisible ? (
      <h2 className="text-lg font-semibold text-gray-100 mb-4">{group.name}</h2>
    ) : (
      <input
        type="text"
        value={newName}
    ...
    <div className="mt-6 space-y-4">
      {Array.isArray(exercises) && exercises.length > 0 ? (
        ...
        <p className="text-sm text-gray-400">
          ...
          <Link href="/dashboard/catalog" className="underline text-blue-400 hover:text-blue-500">
            catalog
          </Link>
          .
        </p>
    ...
    {exercises.length > 1 && (
      <button
        className="mt-4 px-4 py-2 text-sm font-medium text-white bg-blue-500 rounded-lg hover:bg-blue-600 transition-all shadow-md"
